...
bottomline <- ggplot(m[rsq=="[0,0.5]"],aes(x=tested.cv,y=value/number_run,fill=variable)) +
  geom_col() +
  ...
  dt=dcast(
    m[as.numeric(rsq)==1,.(scenario=scen,nsnps,method,tested.cv,variable,value)]
    ,scenario+nsnps+method+tested.cv ~ variable, value.var="value")
...
plotter=function(tag) {
  ...
  ggplot(nodes, aes(x=x,y=y)) +
    ...
    geom_segment(aes(x=x.from,y=y.to+0.3,xend=x.to,yend=y.from-0.3,col=col),data=rdf,
                 size=0.2,col="black",
...
topline=plot_grid(plotlist=list(NULL,topplots[[1]],NULL,topplots[[2]],NULL,topplots[[3]],NULL,topplots[[4]],NULL),
                  ncol=1,
                  ## labs=c("","a","","b","","c","","d",""),
                  rel_heights=c(1,w,1,w,1,w,1,w,2))
...
library(grid)
lines <- lapply(c(0.333,0.545,0.757), function(Y) {
  linesGrob(
  x = unit(c(0,1),"npc"),
  y = unit(c(Y,Y),"npc"),
...
final=ggdraw(both) +
  ...
  draw_grob(lines[[3]])
## both
ggsave("figure-coloc.png",plot=final,height=8,width=8)
